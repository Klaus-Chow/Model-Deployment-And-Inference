...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = self.wconv1_relu(self.wconv1_bn(self.wconv1(x.view(B,-1,1,T))))

        x = self.layer2(x) #1*3*4*70
        out2 = self.wconv2_relu(self.wconv2_bn(self.wconv2(x.view(B,-1,1,T))))

        x = self.layer3(x) #1*3*2*70
        out3 = self.wconv3_relu(self.wconv3_bn(self.wconv3(x.view(B,-1,1,T))))

        x = self.layer4(x) #1*3*1*70

        x = torch.cat((out1,out2,out3,x),1)
